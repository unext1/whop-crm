...
import { getCellKey, getRowHeightValue, parseCellKey } from '~/utils/data-grid';
import type {
  CellPosition,
  ContextMenuState,
  NavigationDirection,
  ...
  SelectionState,
  UpdateCell,
} from '~/components/data-grid/types/data-types';
...
function useDataGrid<TData>({
  ...
  const focusCellWrapper = React.useCallback((rowIndex: number, columnId: string) => {
  ...
  const onColumnFiltersChange = React.useCallback(
    (updater: Updater<ColumnFiltersState>) => {
      const currentState = store.getState();
      const newFilters = typeof updater === 'function' ? updater(currentState.columnFilters) : updater;
      store.setState('columnFilters', newFilters);
    },
    [store],
  );
  ...
  useIsomorphicLayoutEffect(() => {
    const rafId = requestAnimationFrame(() => {
      ...
      // Force update of all row positions after remeasure
      requestAnimationFrame(() => {
    ...
    data.length, // Add data length to trigger remeasure when rows are added/removed

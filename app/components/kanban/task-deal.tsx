import { Building2, Clock, DollarSign, MessageCircle, RotateCcw, User, Users } from 'lucide-react';
    ...
    <li
      onDragStart={(event) => {
        ...
        event.dataTransfer.setData(
          'application/remix-card',
          JSON.stringify({ id, name, columnId, ownerId, boardId, content, assignees, createdAt }),
        );
      }}
      onDragOver={(event) => {
                ...
                <TooltipTrigger asChild>
                  <div className="flex items-center gap-3 text-xs text-muted-foreground cursor-default">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="truncate font-medium text-foreground">${formatCurrency(amount)}</span>
                    </div>
                  </div>
                </TooltipTrigger>
                <TooltipContent side="top" align="start">

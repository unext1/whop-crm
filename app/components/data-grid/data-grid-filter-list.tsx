...
import { generateId } from '~/components/data-table/id';
...
interface DataGridFilterListProps<TData> extends React.ComponentProps<typeof PopoverContent> {
  table: Table<TData>;
  debounceMs?: number;
...
export function DataGridFilterList<TData>({
  table,
  ...
  const [internalFilters, setInternalFilters] = React.useState<ExtendedColumnFilter<TData>[]>([]);
  const [internalJoinOperator, setInternalJoinOperator] = React.useState<JoinOperator>('and');

  const filters = externalFilters ?? internalFilters;
  const joinOperator = externalJoinOperator ?? internalJoinOperator;

  const setFilters = onFiltersChange ?? setInternalFilters;
  const setJoinOperator = onJoinOperatorChange ?? setInternalJoinOperator;

  const debouncedSetFilters = useDebouncedCallback((newFilters: ExtendedColumnFilter<TData>[]) => {
    if (onFiltersChange) {
    ...
    <Sortable value={filters} onValueChange={setFilters} getItemValue={(item) => item.filterId}>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent
          ...
          align="start"
          className="flex w-full max-w-(--radix-popover-content-available-width) flex-col gap-4 p-0 sm:min-w-[420px] bg-muted/30 backdrop-blur-md border-none shadow-lg shadow-s"
          {...props}
        >
          {/* Header */}

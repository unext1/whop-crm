...
async function handleWebhookEvent(action: string, data: Record<string, unknown> | null): Promise<void> {
  ...
  if (action === 'payment.succeeded' || action === 'app_payment.succeeded') {
    ...
    if (product_id === env.WHOP_PREMIUM_PRODUCT_ID && company_id) {
      await handleOrganizationPremiumAccess(company_id, 'premium').catch(() => {
    ...
    if (productId === env.WHOP_PREMIUM_PRODUCT_ID && companyId && membershipId) {
      ...
      if (isAuthorizedUser) {
        ...
        await handleOrganizationPremiumAccess(companyId, 'premium', membershipData).catch(() => {
          // Error activating premium membership
        });
      }
    }
  } else if (
    ...
    if (productId === env.WHOP_PREMIUM_PRODUCT_ID && companyId) {
      if (cancel_at_period_end) {
        ...
        await db
          ...
          .catch(() => {
            // Error marking membership cancellation
          });
      } else {
        // Membership deactivated immediately
        await handleOrganizationPremiumAccess(companyId, 'free').catch(() => {
          // Error deactivating membership
        });
      }
    }
  }
...
async function handleOrganizationPremiumAccess(
  ...
  if (plan === 'premium' && membershipData) {
    ...
    // Set lastMembershipCheck to 5 minutes in the future to skip immediate validation
    // This gives time for the membership to become active in Whop's system
    updateData.lastMembershipCheck = new Date(Date.now() + 5 * 60 * 1000).toISOString();
  } else if (plan === 'free') {

...
export const action = async ({ request }: ActionFunctionArgs): Promise<Response> => {
	...
	if (webhookData.action === "payment.succeeded") {
		const { id, final_amount, amount_after_fees, currency, user_id, company_id, product_id } =
			webhookData.data;
		...
		if (product_id === PREMIUM_PRODUCT_ID && company_id) {
			...
			await handleOrganizationPremiumAccess(company_id, 'premium');
		}
		...
		const { product_id, page_id, id: membershipId } = (webhookData.data as any);
		...
		if (product_id === PREMIUM_PRODUCT_ID && page_id) {
			...
			const membershipData = {
				membershipId,
				renewal_period_start: (webhookData.data as any).renewal_period_start
					? new Date((webhookData.data as any).renewal_period_start * 1000).toISOString()
					: null,
				renewal_period_end: (webhookData.data as any).renewal_period_end
					? new Date((webhookData.data as any).renewal_period_end * 1000).toISOString()
					: null,
			};
			await handleOrganizationPremiumAccess(page_id, 'premium', membershipData);
		}
	} else if (webhookData.action === "membership.went_invalid") {
		const { product_id, page_id } = (webhookData.data as any);

		// Check if this membership deactivation is for the premium product
		if (product_id === PREMIUM_PRODUCT_ID && page_id) {
			// Check if there are any other active premium memberships for the organization
			// If not, downgrade to free
			await checkAndDowngradeOrganization(page_id);
		}
	}
